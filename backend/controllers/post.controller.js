...
export const getAllPost=async (req, res)=>{
    try {
        const post= await Post.find().sort({createdAt: -1}).populate({path:"user", select: "-password"}).populate({path: "comments.user", select: "-password"});
...
export const commentOnPost=async (req, res)=>{
    try {
        ...
        if(!text) return res.status(401).json({error: "Text is required for comment."});
